          ...
          <Row style="height: 100%">
            ...
            <Col style="height: 100%" :span="8">
              <div class="main">
                ...
                <Tabs v-model="loginState.activeName" @tab-click="handleClick">
                  <TabPane name="password">
                    <span slot="label" class="tab-pane-label">密码登录</span>
                    <Form ref="loginForm-password" :model="form.password" :rules="rules.password">
                      <slot name="form-item-prefix"></slot>
                      <FormItem prop="username">
                        <Input v-model="form.password.username" @keyup.enter.native="login"
                               :placeholder="placeholder.username"/>
                      </FormItem>
                      <FormItem prop="password">
                        <Input type="password" v-model="form.password.password" @keyup.enter.native="login"
                               :placeholder="placeholder.password"/>
                      </FormItem>
                      <FormItem prop="verificationCode">
                        <Row :gutter="10">
                          <Col :span="16"><Input v-model="form.password.verificationCode" @keyup.enter.native="login"
                                                 :placeholder="placeholder.verificationCode"/></Col>
                          <Col :span="8">
                            <template class="vc-img">
                              <slot name="form-login-verificationCode">
                                <img class="vc-img" src="../../assets/imgs/login3/verification-code.png"/>
                              </slot>
                            </template>
                          </Col>
                        </Row>
                    ...
                    <slot name="form-login-btn">
                      <Button :loading="loginState.loading" :disabled="loginState.loading" class="login-btn"
                              type="primary" @click="login">登 录
                      </Button>
                  ...
                  <TabPane name="message">
                    ...
                    <Form ref="loginForm-message" :model="form.message" :rules="rules.message">
                      <slot name="form-item-prefix"></slot>
                      <FormItem prop="phone">
                        <Input v-model="form.message.phone" @keyup.enter.native="login"
                               :placeholder="placeholder.phone"/>
                      </FormItem>
                      <FormItem prop="verificationCode">
                        <Input v-model="form.message.verificationCode" @keyup.enter.native="login"
                ...
                <slot name="login-footer">
                  <div class="login-footer">
                    ...
                    <span class="certified" @click="enterpriseCertification">企业认证</span>
                  ...
                  <div class="third-party">
                    <img class="third-party-img" src="../../assets/imgs/login3/weixin.svg" @click="wxLogin"/>
                  </div>
                </slot>
              </div>
            </Col>
      ...
      <div class="footer" v-if="config.footer.show">
        <template v-if="config.footer.copyright">
          <div class="copyright" v-if="config.footer.copyright">
            Copyright &copy; {{ config.footer.copyright.year }} <a :href="config.footer.copyright.href"
                                                                   :target="config.footer.copyright.target">{{
              config.footer.copyright.content
            }}</a>
          </div>
        </template>
      </div>
...
export default {
  ...
  methods: {
    login() {
      this.$refs['loginForm-' + this.loginState.activeName].validate(valid => {
